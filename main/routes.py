...
from webapp.main.forms import JoinForm
import datetime
...
#!joining section
@main.route('/join_request/<shorten>', methods=['POST','GET'])
@login_required
@able_to_join
def join_request(shorten):
    ...
    if form.validate_on_submit():
        current_user.temp_join(user, why_join=form.why_join.data,
            want_to_achieve=form.want_to_achieve.data, willing_to_offer=form.willing_to_offer.data)
        db.session.commit()
    ...
    return render_template('account/join_details.html', title=f'{user.shorten}', heading=f'Join {user.shorten}', form=form, name=f' {current_user.f_name} {current_user.l_name} ')
...
@main.route('/temporal/members/<username>')
@login_required
@role_required('Association')
def review(username):
    user = User.query.filter_by(username=username).first()
    if user is None:
        ...
        return redirect(url_for('main.home'))
    if not user.temp_is_a_member(current_user):
        flash('unauthorized!! user have not made any join request', 'danger')
        return redirect(url_for('main.home'))
    if user.has_role('Association'):
        flash('This user is an association. Therefore cannot Join', 'danger')
        return redirect(url_for('main.home'))
    member = user.temp_association.filter_by(temp_association_id=current_user.id).first()
    return render_template('account/review.html',user=user, member=member)
...
def accept(username):
    ...
    if not current_user.temp_is_association_for(user):
        ...
        return redirect(url_for('main.home'))
    member = user.temp_association.filter_by(temp_association_id=current_user.id).first()
    user.join(current_user, why_join=member.why_join, want_to_achieve=member.want_to_achieve, willing_to_offer=member.willing_to_offer)
...
def reject(username):
    ...
    if user is None:
        ...
        return redirect(url_for('main.home'))
    if not current_user.temp_is_association_for(user):
        flash('You cannot reject a user who have not applied.', 'danger')
        return redirect(url_for('main.home'))
    user.temp_leave(current_user)
    db.session.commit()
    flash('%s has been rejected.' %username, 'success')
...
def make_leave(username):
    ...
    if not current_user.is_association_for(user):
        flash('user cannot leave this association since user have not joined.', 'danger')
        return redirect(url_for('main.home'))
...
@main.route('/active/members/<username>')
@login_required
@role_required('Association')
def view_member(username):
    ...
    if user is None:
        flash('User does not exist.', 'danger')
        return redirect(url_for('main.home'))
    if not user.is_a_member(current_user):
        flash('unauthorized!! user is not a member of this Association', 'danger')
        return redirect(url_for('main.home'))
    if user.has_role('Association'):
        flash('This user is an association. Therefore cannot be a member', 'danger')
        return redirect(url_for('main.home'))
    member = user.association.filter_by(association_id=current_user.id).first()
    return render_template('account/review.html',user=user, member=member)
